...
def save_checkpoint(state, is_best, single=True, checkpoint='checkpoint', filename='checkpoint.pth.tar'):
    ...
    if is_best:
        best_model_name = 'best_model_' + str(state['epoch']) + '_' + str(int(round(state['train_acc']*100, 0))) + '_' + str(int(round(state['acc']*100, 0))) + '.pth'
        best_model_path = os.path.join(checkpoint,'best',fold + model_name)
